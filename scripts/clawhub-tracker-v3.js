#!/usr/bin/env node
/**
 * ClawHub Top 100 Skills Tracker v3.0
 * ===================================
 * 优化版本：并行获取技能详情，提高速度
 */
...
const CONFIG = {
  ...
  USER_AGENT: 'OpenClaw-ClawHub-Tracker/3.0',
  MAX_RETRIES: 2,
  RETRY_DELAY_MS: 1000,
  CONCURRENT_REQUESTS: 10,
  SEARCH_QUERIES: ['a', 'agent', 'tool', 'skill', 'ai', 'auto', 'data', 'web', 'file', 'system', 'github', 'browser', 'doc', 'search']
};
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', () => {
      if (retryCount < CONFIG.MAX_RETRIES) {
        setTimeout(() => httpRequest(url, retryCount + 1).then(resolve).catch(reject), CONFIG.RETRY_DELAY_MS);
      } else {
...
async function fetchWithConcurrency(items, fetchFn, concurrency = 10) {
  const results = [];
...
async function fetchTopSkills(limit = 100) {
  ...
  console.log(`📊 并行获取技能详情（并发：${CONFIG.CONCURRENT_REQUESTS}）...`);
  
  const slugs = Array.from(allSkillsMap.keys());
  const skillsDetails = await fetchWithConcurrency(slugs, fetchSkillDetails, CONFIG.CONCURRENT_REQUESTS);
  
  const validSkills = skillsDetails.filter(s => s && s.stats).map((details, index) => {
  ...
  validSkills.sort((a, b) => b.downloads - a.downloads);
  
  const topSkills = validSkills.slice(0, limit).map((skill, index) => ({
    ...skill,
    rank: index + 1
  }));
  
  console.log(`✅ 成功获取 ${topSkills.length} 个技能`);
  return topSkills;
}
...
async function main() {
  ...
  try {
    console.log('🚀 ClawHub Top 100 Tracker v3.0');
