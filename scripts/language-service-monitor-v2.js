...
function loadLearningLog() {
  try {
    ...
    const learning = { failedSites: [], successfulPatterns: [], optimizationNotes: [] };
    ...
    for (const line of lines) {
      ...
      else if (line.startsWith('- ') && currentSection) learning[currentSection].push(line.slice(2));
    }
    return learning;
  } catch (err) {
...
function saveLearning(learning) {
  ...
  content += `## 失败网站\n\n`;
  for (const site of learning.failedSites) content += `- ${site}\n`;
  content += `\n## 成功模式\n\n`;
  for (const pattern of learning.successfulPatterns) content += `- ${pattern}\n`;
  content += `\n## 优化建议\n\n`;
  for (const note of learning.optimizationNotes) content += `- ${note}\n`;
...
function summarizeContent(url, title, htmlContent = "") {
  try {
    ...
    if (htmlContent && htmlContent.length > 100) {
      const text = htmlContent.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 2000);
      const cmd = `bash "${summarizeScript}" bullet "${text}" 2>&1`;
      ...
      if (keyPoints.length > 0) return keyPoints.map(l => l.replace(/^\s*\d+\.\s*/, '')).join(' ');
    }
...
function httpRequest(url, followRedirect = true) {
  ...
  return new Promise((resolve, reject) => {
    https.get(url, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; LanguageServiceMonitor/2.1)' },
      timeout: 10000
    }, (res) => {
...
function loadLastCheck() {
  ...
  catch (err) { return { lastCheck: null, articles: [] }; }
}
...
function generateMarkdownReport(articles, timestamp) {
  ...
  let report = `# 语言服务动态监控周报_${dateStr} (v2.1)\n\n`;
  report += `**生成时间:** ${timestamp}\n\n`;
  report += `**增强功能:** doc-summarize-pro 智能摘要 + proactive-agent 趋势发现 + self-improving 学习优化\n\n---\n\n`;
  
  const byPriority = { high: [], normal: [], low: [] };
  for (const article of articles) byPriority[article.priority || 'normal'].push(article);
  ...
  if (articles.length === 0) report += `## 暂无更新\n\n本次检查未发现新的新闻动态。\n\n`;
...
async function main() {
  ...
  for (const org of MONITOR_CONFIG.organizations) {
    try {
      ...
      for (const article of newNews) {
        console.log(`    🧾 生成摘要...`);
        const summary = summarizeContent(article.url, article.title, html);
        newArticles.push({ ...article, source: org.name, type: 'organization', summary, isMajor: org.priority === 'high', priority: org.priority });
      }
  ...
  for (const company of MONITOR_CONFIG.companies) {
    try {
      ...
      for (const article of newNews) {
        const summary = summarizeContent(article.url, article.title, html);
        newArticles.push({ ...article, source: company.name, type: 'company', summary, priority: 'normal' });
      }
  ...
  if (emergingCompanies.length > 0) {
    ...
    for (const c of emergingCompanies) console.log(`    - ${c.company} (${c.count} 条 AI/ML 相关新闻)`);
  }
  
  const prioritizedArticles = prioritizeArticles(newArticles, learning);
  const newState = { lastCheck: new Date().toISOString(), articles: [...lastState.articles, ...newArticles].slice(-100) };
  saveState(newState);
