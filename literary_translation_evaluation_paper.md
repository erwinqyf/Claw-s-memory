...
2. **评估粒度敏感度**：同一译文在段落级 vs 句子级评估下的表现差异
...
本实验在**段落级（label）**和**句子级（sentence）**两个粒度上评估了 6 个模型配置生成的翻译译文。实验使用相同的源文本（文学文本），每个模型配置独立生成译文，然后对每个译文分别进行段落级和句子级评估。

**实验设计**：
- **源文本**：相同的文学文本（中文）
- **译文生成**：6 个模型配置 × 相同源文本 = 6 份独立译文
...
*注：排名差异反映了不同模型在段落级 vs 句子级评估下的表现差异。原文相同，6 个模型配置生成独立译文，每份译文分别进行两种粒度评估。*
