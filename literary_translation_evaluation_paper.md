...
Zhang et al. (2020) [26] 在ICLR发表的BERTScore研究表明，基于预训练语言模型上下文嵌入的语义相似度评估，与人类判断的相关性显著高于传统的n-gram匹配指标。BERTScore通过计算候选译文与参考译文在嵌入空间的余弦相似度，能够识别词汇变体、同义替换和语序调整等合理改写，避免BLEU对语义等价但形式不同表达的惩罚。这一特性使其特别适合评估文学翻译中常见的创造性改写和意境传达。

**BLEU低权重的理论依据：**

Toral & Way (2018) [12] 在文学翻译MT评估的开创性研究中指出，传统自动指标在评估文学文本时存在系统性偏差：过度关注词汇层面的精确匹配，而忽视风格、韵律、文化负载词等文学性要素。Moorkens et al. (2018) [27] 进一步证实，BLEU等n-gram指标会惩罚文学翻译中必要的创造性改写，导致对高质量文学译文的低估。

**语义优先指标组合的理论依据：**

Wong & Kit (2010) [28] 在LREC发表的语义评估研究表明，将词汇语义相似度度量整合到MT评估框架中，能够显著改善对同义词对的识别能力，减少因词汇选择差异导致的质量低估。这一发现支持了本研究采用多语义指标（BERTScore、Cosine、METEOR）组合的策略。

Vieira et al. (2025) [25] 的最新研究系统评估了各类指标在文学翻译上的表现，发现即使LLM-based指标在捕捉风格保真度和文化适应性方面仍有局限，但语义导向的评估方法明显优于传统的词汇匹配方法。这为本研究的语义优先加权策略提供了实证支持。
...
[27] Moorkens, J., et al. (2018). Translation quality and productivity: The impact of translation memory matches and machine translation. *Translation Spaces*, 7(2), 207-223.

[28] Wong, B. T. M., & Kit, C. (2010). Semantic evaluation of machine translation. *LREC 2010*.
