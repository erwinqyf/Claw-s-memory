...
本研究系统评估了三种大规模语言模型（TranslateGemma-12B、TranslateGemma-27B、HunyuanMT-7B）在文学翻译任务上的性能表现。通过统一温度参数（temperature=0.75）并对比 top-p 采样策略（开启 top-p=0.9 vs 不开启）的影响，采用 11 项自动评估指标进行多维度质量分析。实验结果表明：（1）TranslateGemma-27B 在段落级任务中表现最优（加权得分 0.4190），而 HunyuanMT-7B 在句子级任务中领先（加权得分 0.4622）；（2）top-p 采样的影响存在模型特异性，大模型（27B）受益于多样性增强，而专用模型（HunyuanMT）在不使用 top-p 时更稳定；（3）基于 BERTScore（0.25）、Cosine（0.15）、BLEU（0.15）等多维加权的评估体系有效平衡了语义理解与字面约束，METEOR（0.10）和 ROUGE-L（0.10）进一步捕捉了词汇多样性和连贯性。本研究提出的九指标加权策略为文学翻译自动评估提供了新的方法学参考。
...
1. **语义优先（BERTScore 0.25）**：文学翻译重在"神似"，语义理解权重最高
2. **多维度平衡**：词频（Cosine）、n-gram（BLEU）、同义词（METEOR）、连贯性（ROUGE-L）、神经评估（COMET）形成互补
3. **适度字面约束（BLEU 0.15）**：避免过度意译导致偏离原文
4. **错误惩罚（TER/WER -0.10）**：确保翻译的基本准确性底线
5. **基础对齐（Match Rate 0.05）**：保留最低限度的词汇对应要求

**与传统评估的区别：**

| 对比维度 | 传统 MT 评估 | 本研究（文学翻译） |
|:---------|:-------------|:-------------------|
| 核心指标 | BLEU（权重 0.40-0.60） | BERTScore（权重 0.25） |
| 评估重点 | 字面匹配、n-gram 精确率 | 语义相似、深层理解 |
| 对意译的容忍度 | 低 | 高（BLEU 仅 0.15） |
| 指标数量 | 3-5 项 | 9 项（含 2 项负向） |
| 设计目标 | 技术文档翻译质量 | 文学翻译的"神似" |
...
3. **文学性专项**：对于诗歌、散文等高度文学化文本，建议调整权重：
   - 提高 BERTScore 至 0.30-0.35（强化语义理解）
   - 降低 BLEU 至 0.05-0.10（放宽字面约束）
   - 提高 METEOR 至 0.15（重视同义词匹配）
4. **技术文档**：对于术语密集型文本，建议调整权重：
   - 提高 BLEU 至 0.25-0.30（强化术语一致性）
   - 提高 Match Rate 至 0.10-0.15（重视词汇对齐）
   - 降低 BERTScore 至 0.20（适度语义容忍）
5. **最终质检**：人工评估风格保真度和文化适应性
...
1. **方法学贡献**：
   ...
   - **提出了语义优先的九指标加权策略**：BERTScore 0.25（最高权重）+ Cosine/BLEU 0.15 + METEOR/ROUGE-L/COMET 0.10 + Match Rate 0.05 - TER/WER 0.10，体现文学翻译"神似重于形似"的评估理念
