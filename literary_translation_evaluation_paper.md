...
本研究系统评估了三种大规模语言模型（TranslateGemma-12B、TranslateGemma-27B、HunyuanMT-7B）在文学翻译任务上的性能表现。通过统一温度参数（temperature=0.75）并对比 top-p 采样策略（开启 top-p=0.9 vs 不开启）的影响，采用 11 项自动评估指标进行多维度质量分析。实验结果表明：（1）TranslateGemma-27B 在段落级任务中表现最优（加权得分 0.4190），而 HunyuanMT-7B 在句子级任务中领先（加权得分 0.4622）；（2）top-p 采样的影响存在模型特异性，大模型（27B）受益于多样性增强，而专用模型（HunyuanMT）在不使用 top-p 时更稳定；（3）基于 BERTScore（0.25）、Cosine（0.15）、BLEU（0.15）等多维加权的评估体系有效平衡了语义理解与字面约束，成功识别了"神似"优于"形似"的翻译；（4）权重设计验证有效：BERTScore 高的模型（如 TranslateGemma-27B topp=0.9，BERTScore=0.7842）获得最高加权得分，而单纯 BLEU 高的模型（如 HunyuanMT-7B 无topp，BLEU=0.4349）排名次之。本研究提出的九指标加权策略为文学翻译自动评估提供了从"形似"向"神似"转变的方法学参考。
