...
本研究系统评估了三种大规模语言模型（TranslateGemma-12B、TranslateGemma-27B、HunyuanMT-7B）在文学翻译任务上的性能表现。通过统一温度参数（temperature=0.75）并对比 top-p 采样策略（开启 top-p=0.9 vs 不开启）的影响，采用 11 项自动评估指标进行多维度质量分析。实验结果表明：（1）TranslateGemma-27B 在段落级任务中表现最优，而 HunyuanMT-7B 在句子级任务中领先；（2）top-p 采样的影响存在模型特异性，大模型（27B）受益于多样性增强，而专用模型（HunyuanMT）在不使用 top-p 时更稳定；（3）chrF、TER 和 BLEU 对模型差异最敏感，而 COMET 和 BERTScore 相对稳定。本研究为文学翻译自动评估提供了方法学参考，并揭示了解码参数对翻译质量的影响机制。
...
1. **快速筛选**：使用 COMET + BLEU 双指标组合，可覆盖 80% 以上的质量差异
2. **深度评估**：增加 chrF 和 METEOR，捕捉词汇多样性
3. **最终质检**：人工评估风格保真度和文化适应性
...
1. **方法学贡献**：构建了涵盖词汇、语义、错误率三个层面的 11 指标综合评估框架，为文学翻译自动评估提供参考
2. **实证发现**：
...
3. **指标分析**：识别了 chrF、TER、BLEU 等高敏感度指标，以及 COMET-BERTScore、TER-WER 等高冗余指标对
...
1. **引入人工评估**：建立文学翻译的多维度人工评估框架，验证自动指标的有效性
2. **扩展模型范围**：纳入 GPT-4、Claude 等商业模型，构建更全面的基准
3. **探索提示工程**：研究系统提示（system prompt）对文学翻译质量的影响
4. **开发专用指标**：基于 LiTransProQA[14] 等框架，设计针对文学翻译的自动评估指标
5. **跨语言验证**：在更多语言对（如中日、中法）上验证结论的普适性
6. **参数优化**：通过网格搜索识别不同模型-任务组合的最优解码参数
