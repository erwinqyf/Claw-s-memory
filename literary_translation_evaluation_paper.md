...
**重要说明**：本研究的翻译文本在所有实验条件下保持一致，仅评估粒度不同：
- **段落级评估（Label）**：将翻译文本按段落分割后评估，捕捉长距离依赖和篇章连贯性
- **句子级评估（Sentence）**：将翻译文本按句子分割后评估，关注局部语义准确性

这种设计确保了评估结果的可比性——差异仅源于评估粒度，而非翻译内容本身。
...
本实验在**段落级（label）**和**句子级（sentence）**两个粒度上评估了 6 个模型配置的翻译质量。所有模型统一使用 temperature=0.75，其中 3 个配置开启 top-p=0.9，3 个配置不开启 top-p。

**关键设计**：翻译文本在所有条件下保持一致，评估粒度的差异（段落级 vs 句子级）反映了指标对长距离依赖和局部语义的不同敏感度。
...
**重要说明**：由于翻译文本相同，这一分歧反映了**评估粒度对质量判断的影响**，而非翻译内容本身的差异：

- **段落级评估**：捕捉长距离依赖，TranslateGemma-27B 的篇章连贯性优势显现
- **句子级评估**：关注局部语义，HunyuanMT-7B 的句子级准确性占优
...
*注：排名差异反映了评估粒度对质量判断的影响。翻译文本相同，仅评估粒度不同。*
